...
resource "aws_security_group" "changeme_aws_security_group_efs" {
...
resource "aws_efs_mount_target" "changeme_efs_mount_target" {
  ...
  security_groups = [aws_security_group.changeme_aws_security_group_efs.id]
}


# Documentation: https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/efs_access_point
resource "aws_efs_access_point" "changeme_efs_access_point" {
  file_system_id = aws_efs_file_system.changeme_aws_efs.id
}
