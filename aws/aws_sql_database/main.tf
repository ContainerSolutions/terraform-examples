...
provider "aws" {
  ...
  default_tags {
    tags = {
      cs_terraform_examples = "aws_sql_database"
    }
  }
}

# Documentation: https://registry.terraform.io/providers/hashicorp/random/latest/docs/resources/db_instance
...
# Allocates a MySQl version 5.7 named 'change_mydb' in a t3.micro instance with a database name 'changeme_mydb', username 'changeme_username' and passwd 'changeme_password'
resource "aws_db_instance" "changeme_db_instance" {
  allocated_storage   = 5
  engine              = "mysql"
  engine_version      = "5.7"
